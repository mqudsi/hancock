...
functor PPAstFn (structure PPAstAdornment : PPASTADORNMENT) : PP_AST = struct 
  ...
  (* HANCOCK *)
  (* Take a comma expression and pull out a list of all the comma  separated subterms. *)
  fun extract_comma_exps (e as EXPR(coreExpr,_,_))  =
    case coreExpr of
       Comma (e0,e1) => (extract_comma_exps e0) @ (extract_comma_exps e1)
     | _ => [e]
  (* HANCOCK *)
  ...
  and ppCoreStmt aidinfo tidtab pps coreStmt = 
    case coreStmt
      of Expr expOpt => 
       ...
       | CommentStmt s => (* HANCOCK: used to generate internal comments. *)
	   ( PPL.addStr pps "/*"
	   ; PPL.addStr pps s
           ; PPL.addStr pps "*/"
	   )
  ...
  and ppCoreExpr {nested} aidinfo tidtab pps coreExpr = 
    case coreExpr
      of IntConst li => PPL.ppLLI pps li
       ...
       | Assign (e0,e1) =>
	   ( ppLParen nested pps 
	   ; ppExpr {nested=true} aidinfo tidtab pps e0  (* HANCOCK: changed to true. *)
	   ; PPL.addStr pps " = "
       ...
       | Comma (e0,e1) =>
	   (* HANCOCK *)
	   let val exps = (extract_comma_exps e0 @ extract_comma_exps e1)
	       fun pp' pps e = (ppExpr {nested=true} aidinfo tidtab pps e)
	       fun pp_f pps = (PPL.addStr pps ","; PPL.newline pps)
	       fun pr_block es =
		   ( PPL.addStr pps "("
		    ; PPL.blockify 2 (PPL.separate (pp', pp_f)) pps es
		    ; PPL.addStr pps ")")
	       fun pr_list es = 
		   PPL.ppList { pp=ppExpr {nested=true} aidinfo tidtab
			       , sep=","
			       , lDelim="("
			       , rDelim=")"
			       } pps es
	   in
	       if List.length exps > 2 then pr_block exps else pr_list exps
	   end
(*	   ( PPL.addStr pps "("
...
*)
  ...
  fun ppCoreExternalDecl aidinfo tidtab pps edecl =
    case edecl
	  ...
	  ppDeclaration aidinfo tidtab pps decl
       ...
       | Directive s => (PPL.addStr pps s) (* HANCOCK *)
       | ExternalComment s =>  (* HANCOCK: used for internal comments. *)
	   ( PPL.addStr pps "/*"
	   ; PPL.addStr pps s
	   ; PPL.addStr pps "*/"
	   )
  ...
  fun ppExternalDeclRefined srcFile aidinfo tidtab pps edecl = 
       PPAA.ppExternalDeclAdornment srcFile ppCoreExternalDecl aidinfo tidtab pps edecl

  fun ppAst srcFile aidinfo tidtab pps edecls = 
      PPL.separate (ppExternalDeclRefined srcFile aidinfo tidtab,PPL.newline) pps edecls
  ...
  val ppAst = fn srcFile => wrap (ppAst srcFile) (*Hancock *)
end
