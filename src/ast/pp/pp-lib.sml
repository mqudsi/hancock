structure PPLib = struct
  ...
  fun ppLLI pps i = 
      let fun ppPosLong pps i = 
	      let val maxInt32Str = Int32.toString(valOf (Int32.maxInt))
		  val maxInt32Size = String.size maxInt32Str
		  val iAsStr = IntInf.toString i
		  val numDigits = String.size iAsStr
		  val needsLL = if numDigits < maxInt32Size then false
		                else if numDigits > maxInt32Size then true
				else String.>(iAsStr,maxInt32Str)
	      in
		  (addStr pps (IntInf.toString i);
		   if needsLL then addStr pps "LL" else ())		   
	      end
      in
	 if IntInf.>=(i,IntInf.fromInt 0) 
         then ppPosLong pps i
	 else (addStr pps "-"; ppPosLong pps (IntInf.~(i)))
      end
