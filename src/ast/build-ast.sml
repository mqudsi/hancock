...
struct
  ...
  val default_signed_char = ref true (* ksf: HANCOCK : July 14, 2000*)
...
let
  ...
  (* HANCOCK : Extensions in Parse Tree no CnvExt ! *)
  val refCNVExp = ref(convFunError "CNVExp" : PT.expressionExt -> Ast.ctype * Ast.expression)
  val refCNVStat = ref(convFunError "CNVStat": PT.statementExt -> Ast.statement)
  ...
  val refCNVUnop = ref(convFunError "CNVUnop": {unop: ParseTreeExt.operatorExt, argExpr: ParseTree.expression}
                                                -> Ast.ctype * Ast.expression)
  val refCNVExternalDecl = ref(convFunError "CNVExternalDecl" : PT.externalDeclExt -> Ast.externalDecl list)
  val refCNVSpecifier = ref(convFunError "CNVSpecifier": {isShadow: bool, rest : ParseTree.specifier list} 
			                                -> PT.specifierExt
			                                -> Ast.ctype)
  val refCNVDeclarator = ref(convFunError "CNVDeclarator":  Ast.ctype * PT.declaratorExt 
			                                 -> Ast.ctype * string option)
  val refCNVDeclaration = ref(convFunError "CNVDeclaration": PT.declarationExt -> Ast.declaration list)
...
(* HANCOCK *)
  val mkFunctionCt  = TU.mkFunctionCt ttab error 
(*  fun mkFunctionCt (retTy, argTys) =
  ...
  fun TCInitializer(ctype as (Ast.TypeRef _ | Ast.Qual _), expr) =
	...
	if isScalar ty then
	  ...
	  error "illegal aggregate initializer 1"	  
	  
    | TCInitializer (_, Ast.Aggregate _) =
	error "illegal aggregate initializer 2"
		  ...
		  fun addQual q = {qualifiers=q::qualifiers, specifiers=specifiers}
	       in case decr
			...
			(pushLoc loc;
			 ...
			 before popLoc ())
		     (* HANCOCK *)
		     | PT.DecrExt (ParseTreeExt.HWindow (x,sz,offset)) =>
			vardeclToTypeNameLoc(mkTyp(PT.SpecExt(ParseTreeExt.HWindowSpec(sz,offset,typ))),
					     x)
		     (* HANCOCK end *)
  ...
  *)
   ...
   * -------------------------------------------------------------------- *)
(* HANCOCK *)
(* Last thing done when converting an external decl. *)
  and finishExternal d = 
      let val pt = List.rev (HState.PTpopExtDecls ())
	  val d' = List.concat (List.map cnvExternalDecl pt)
      in
	  ((HState.popExtDecls ()) @ d' @ d)
      end
				 (* HANCOCK *)
  ...
  and cnvExternalDecl (PT.ExternalDecl(PT.DeclarationExt ext)) =
    ...
    | cnvExternalDecl (PT.ExternalDecl(PT.Declaration(dt as {qualifiers, specifiers, storage},
	  ...
	  in  (* global typedefs *)
		  ...
		  in 
		      finishExternal  (* HANCOCK *)
		      (List.map 
		       (fn x => wrapDECL(Ast.ExternalDecl(Ast.TypeDecl
							  {shadow=NONE, tid=x}
							  ))) tidl)
		  end
	  end
      ...
      in if isShadow
	    then let fun getTid (Ast.StructRef tid) = SOME({strct=true}, tid)
		 ...
		 in
		   case getTid ty of
		     SOME(strct, tid) => 
			 finishExternal (* HANCOCK *)
			 [wrapDECL(Ast.ExternalDecl(Ast.TypeDecl{shadow=SOME strct, tid=tid}))]
		   | NONE => []
		 end
	  else
	    ...
	    in 
		finishExternal (* HANCOCK *)
		(List.map (fn x => wrapDECL(Ast.ExternalDecl(Ast.VarDecl x))) 
		 idExprs)
	    end
      end
    ...
    | cnvExternalDecl (PT.ExternalDeclExt extDecl) =
       CNVExternalDecl extDecl
    | cnvExternalDecl (PT.Directive s) = 
       if !HFlags.preserveLineNums then [wrapDECL (Ast.Directive s)] 
       else [](* HANCOCK *)
    | cnvExternalDecl (PT.ExternalComment s) =
       [wrapDECL (Ast.ExternalComment s)] (* HANCOCK *)
  ...
  and processDecls ((PT.Decl decl) :: rest, astdecls: Ast.declaration list list)
	  ...
	  if isTYPEDEF dt then
	      let 
		  val ct = {qualifiers=qualifiers, specifiers=specifiers}
	      ...
	      let 
		  val isShadow = List.null declExprs andalso isTagTy dt
		  val (ty,sc) = cnvType (isShadow, dt)
  ...
  and cnvStatement (stmt: PT.statement): Ast.statement =
	...
	| PT.CaseLabel (expr, stmt) =>
	   let val n = case expr of
	     PT.EmptyExpr => (error "Non-constant case label."; IntInf.fromInt 0)
	   | _ => (case evalExpr expr of  (* cannot be EmptyExpr *)
		     (SOME i, _, _, sizeofFl) =>
			  ...
			  then warn("sizeof in case label not preserved in source-to-source mode.")
		       ...
		       i
                        handle OverFlow => (error("Case label too large.");  IntInf.fromInt 0))
		   | (NONE, _, _, _) => (error "Non-constant case label."; IntInf.fromInt 0))
	   in case addSwitchLabel (Int32.fromInt (IntInf.toInt n))
	     of NONE => ()
	   | SOME msg => error msg;
	...
	| PT.StatExt stmt =>
	   CNVStat stmt
	| PT.CommentStmt s => (wrapSTMT (Ast.CommentStmt s)) (* HANCOCK *)
			...
			(case tagOpt
			  ...
			  let
			    ...
			    fun process2 (decr,expr)
				 : Ast.ctype * Ast.member option * IntInf.int option = 
			      let
