...
(* follows "ISO C conversion" column of table 6-5 in Haberson/Steele, p176
 "C Reference Manual", 4th Ed *)
  ...
  fun preArgConv tidtab ty =
      (* HANCOCK : FMS: I believe this is the right place to check 
       convert_function_args_to_pointers. *)
      (if TypeCheckControl.convert_function_args_to_pointers then
	   ...
	   (cnvFunctionToPointer2Function tidtab ty))
...
(* HANCOCK : Moved mkFunctionCT here from build-ast.sml . *)
  fun mkFunctionCt ttab error (retTy, argTys) =
      (if isNonPointerFunction ttab retTy
	   then error "Return type of function cannot be function type."
       else ();
       if isArray ttab retTy
	   then error "Return type of function cannot be array type."
       else ();
       let val argTys = 
	   if TypeCheckControl.convert_function_args_to_pointers then
	       List.map (preArgConv ttab) argTys
	   else List.map (cnvFunctionToPointer2Function ttab) argTys
