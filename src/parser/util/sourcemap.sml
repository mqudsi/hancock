...
struct
  ...
  (* HANCOCK start *)
  (* FMS: make file names more legible! *)
  fun relativeFileName (file:string) =
      (let val rel = (OS.Path.mkRelative({path=file, relativeTo=OS.FileSys.getDir()}))
       in
	   if (String.size rel) > (String.size file) then file else rel
       end handle OS.Path.Path => file)
  (* HANCOCK end *)
  ...
  fun locToString UNKNOWN = "\"???\""
	...
	val srcFile = relativeFileName srcFile (* HANCOCK *)
