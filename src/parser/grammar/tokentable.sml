    ...
    app ins ([
		...
		[
		 ...
		 ("to" ,         Tokens.TO)
	       ...
	       else (if ParseControl.Hkeywords then 
			 [
			  ("compress"  , Tokens.COMPRESS),
			  ("decompress", Tokens.DECOMPRESS),
			  ("directory" , Tokens.DIRECTORY),
			  ("event"     , Tokens.EVENT),
			  ("exists"    , Tokens.EXISTS),
			  ("filteredby", Tokens.FILTEREDBY),
			  ("initially",  Tokens.INITIALLY),
			  ("iterate"   , Tokens.ITERATE),
			  ("initially" , Tokens.INITIALLY),
			  ("map"       , Tokens.MAP),
			  ("munion"    , Tokens.MUNION),
			  ("new"       , Tokens.NEW),
			  ("over"      , Tokens.OVER),
			  ("pickle"    , Tokens.PICKLE),
			  ("range"     , Tokens.RANGE),
			  ("sig_main"  , Tokens.SIG_MAIN),
			  ("sortedby"  , Tokens.SORTEDBY),	  
			  ("sorted"    , Tokens.SORTED),	  
			  ("split"     , Tokens.SPLIT),	  
			  ("stream"    , Tokens.STREAM),
			  ("value"     , Tokens.VALUE),
			  ("view"      , Tokens.VIEW),
			  ("withevents", Tokens.WITHEVENTS)
			  ]
		     else nil)))
