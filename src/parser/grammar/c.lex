...
fun mkHexInt (s,a,b,errWarn:errWarn)=((case (StringCvt.scanString (IntInf.scan StringCvt.HEX) 
                                                                  (String.extract(s,2,NONE))) of
		  SOME i =>  i  
...
<INITIAL,C>{newline}	=> (SourceMap.newline sourceMap yypos; continue());
...
<S>{newline}	=> ((#err errWarn) (!stringstart,yypos,"unclosed string");
		    SourceMap.newline sourceMap yypos;
		    YYBEGIN INITIAL; Tokens.STRING(makeString charlist,!stringstart,yypos));
<S>[^"\\\010\013]*  => (addString(charlist,yytext); continue());
<S>\\{newline}  => (SourceMap.newline sourceMap yypos; continue());
...
<INITIAL>"$"            => (Tokens.DOLLAR(yypos,yypos+1) (* HANCOCK *));
<INITIAL>"@"            => (Tokens.AT(yypos,yypos+1));
<INITIAL>"<:"           => (Tokens.LCOLON(yypos,yypos+2));
<INITIAL>":>"           => (Tokens.RCOLON(yypos,yypos+2));
<INITIAL>"{:"           => (Tokens.LBCOLON(yypos,yypos+2));
<INITIAL>":}"           => (Tokens.RBCOLON(yypos,yypos+2));
<INITIAL>"(:"           => (Tokens.LPCOLON(yypos,yypos+2));
<INITIAL>":)"           => (Tokens.RPCOLON(yypos,yypos+2));
<INITIAL>"::"           => (Tokens.COLONCOLON(yypos,yypos+2));
<INITIAL>"<=>"          => (Tokens.LTEG(yypos,yypos+3));
<INITIAL>":=:"          => (Tokens.CEC(yypos,yypos+3));
<INITIAL>":+:"          => (Tokens.CPC(yypos,yypos+3));
<INITIAL>":\\:"         => (Tokens.CSC(yypos,yypos+3));
<INITIAL>":-:"          => (Tokens.CMC(yypos,yypos+3));
<INITIAL>"\\"           => (Tokens.SLASH(yypos,yypos+2));
