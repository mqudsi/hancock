...
#endif
{
	...
	int		decimal, thousand, n_thousand, n_dot, n_exponent;
...
#define SFUNGETC(f,c)	(--d)
	...
	n_assign = n_input = decimal = thousand = 0;
...
loop_fmt:
	...
	loop_flags:	/* LOOP FOR FLAGS, WIDTH, BASE, TYPE */
		...
		{	val = accept;
			...
			n_dot = n_exponent = n_thousand = -1;
			dot = decimal ? decimal : '.';
			do
			{	if(isdigit(inp))
				{	if(n_thousand >= 0 && (n_thousand += 1) > 3)
						break;
					*val++ = inp;
				}
				else if(inp == dot)
				{	if(n_thousand >= 0 && n_thousand != 3)
						break;
					n_thousand = -1;

					if(n_dot++ >= 0) /* too many dots */
						break;
				...
				else if(thousand && inp == thousand)
				{	if(n_exponent >= 0 || n_dot >= 0 ||
					   (n_thousand < 0 && (val-accept) > 3) )
						break;
					if(n_thousand >= 0 && n_thousand != 3)
						break;
					n_thousand = 0;
...
#endif
				if(flags&SFFMT_THOUSAND)
					argv.d = (double)_sfstrtod(accept,NIL(char**));
				else	argv.d = (double)strtod(accept,NIL(char**));
			}
		...
		{ int_cvt:
			...
			{	/* fast base 10 conversion */
				...
				{	n_thousand = -1;
					...
					for(sp = accept; sp < val; sp++)
					  argv.lu = (argv.lu<<3)+(argv.lu<<1)+(*sp-'0');
				}
